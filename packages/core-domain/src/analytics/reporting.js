...
export function kpisFrom(exposures, outcomes, days=7){
...
export function convByPriceFrom(exposures, outcomes, sku, buckets=6){
  ...
  const points = bins.map(b => ({ from: round2(b.from), to: round2(b.to), shows: b.shows, wins: b.wins, conversion: b.shows ? round2(b.wins / b.shows) : 0 }));
  return { sku, range:{ min: round2(minP), max: round2(maxP) }, points };
}

export function algoMixFrom(exposures, outcomes, sku){
  const ex = exposures.filter(e=>!sku || e.sku===sku);
  const byModel = {};
  for (const e of ex) {
  ...
  const rows = Object.entries(byModel).map(([model,s])=> ({ model, shows:s.shows, wins:s.wins, win_rate: s.shows? round2(s.wins/s.shows):0, avg_price: s.shows? round2(s.sum_price/s.shows):0 }));
  return { sku, rows };
}
